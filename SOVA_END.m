...
for sym_cnt=  1:num_sym  
   [F_path_metric(:,sym_cnt+1),index] = min([F_path_metric(P_State(:,1),sym_cnt)+ branch_metric(Ga_Inx(:,1),sym_cnt)-(C(sym_cnt)+apr_LLR(sym_cnt)/2) ...
       F_path_metric(P_State(:,2),sym_cnt)+ branch_metric(Ga_Inx(:,2),sym_cnt)-(C(sym_cnt)-apr_LLR(sym_cnt)/2)],[],2);
   survivor_node(:,sym_cnt) = P_State_trans(index+index_temp);
   survivor_ip(:,sym_cnt) = P_Ip_trans(index+index_temp); 
  
   [B_path_metric(:,num_sym+1-sym_cnt),~] = min([B_path_metric(N_State(:,1),num_sym+2-sym_cnt)+ branch_metric(Gb_Inx(:,1),num_sym+1-sym_cnt)-(C(num_sym+1-sym_cnt)+apr_LLR(num_sym+1-sym_cnt)/2) ...
       B_path_metric(N_State(:,2),num_sym+2-sym_cnt)+ branch_metric(Gb_Inx(:,2),num_sym+1-sym_cnt)-(C(num_sym+1-sym_cnt)-apr_LLR(num_sym+1-sym_cnt)/2)],[],2);
end 
...
com_metric = min(F_path_metric(:,bk_cnt)+branch_metric(Gb_Inx(:,com_ip),bk_cnt)-(C(bk_cnt)+((-1)^(com_ip-1))*apr_LLR(bk_cnt)/2) + B_path_metric(N_State(:,com_ip),bk_cnt+1) );
